...
![*Map of % of Female Representatives by State*](https://github.com/lmchu1/WomenLeaders599/blob/master/images/Map.png)
...
```{r, warning=FALSE}
pander(margins.oglmx(results.oprob,ascontinuous = TRUE))
```
...
pander(summary(results.oprobhet))
```

```{r}
results.oprobhet<-oglmx(opinion_leaders ~ perc_women + educ + age + gender + hhincome + employment + party, data=womendata,constantMEAN= FALSE, constantSD= FALSE,threshparam=NULL)   

pander(summary(results.oprobhet$Hetero))
```
